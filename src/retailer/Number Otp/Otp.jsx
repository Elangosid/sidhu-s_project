    ...
    if (!allFieldsFilled) {
      ...
      if (enteredOTP === storedOTP) {
        ...
        try {
          ...
          const retailerCollection = mongodb
            .db('QUIKFLO')
            .collection('Retailer');
          const existingRetailer = await retailerCollection.findOne({
            MobileNo: mobileNumber,
          });

          if (existingRetailer) {
            localStorage.setItem('mobileNumberId', existingRetailer._id);
            console.log('Mobile number already exists:', mobileNumber);
            if (existingRetailer.KYC && existingRetailer.StoreDetails) {
